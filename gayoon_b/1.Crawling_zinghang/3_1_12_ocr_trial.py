# easyocr 성능 높이기 : 이미지 전처리 작업 중요
# 강조 텍스트와 설명 텍스트 분리 (중요한 발견)
import easyocr
import numpy as np
...
def preprocess_image(image_path):
    try:
        ...
        # 명암 대비 강화
        img = ImageEnhance.Contrast(img).enhance(2.8)  # 명암 대비 강화

        # 선명화 필터 적용
        img = img.filter(ImageFilter.SHARPEN)  # 샤프닝 적용

        # OpenCV로 변환
        img_cv = np.array(img)

        # 텍스트 강조를 위한 HSV 변환 및 밝기 조정
